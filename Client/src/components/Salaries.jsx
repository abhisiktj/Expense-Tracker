...
const SalaryForm=()=>{
    ...
    const formik=useFormik({
        ...
        onSubmit:async(values)=>{
             const url="/api/api/v1/salary/addsalary"
    ...
    return(
        ...
        <p className="text-3xl text-blue-900 p-2 m-1">Add a Salary</p>
...
const SalaryList=()=>{
  ...
  const handleNext=()=>{
    ...
    useEffect(()=>{
      const url=`/api/api/v1/salary/getallsalaries/?limit=5&page=${page}/`
      getData(url);
    },[page])
      ...
      return( 
        <div className="flex flex-col justify-center items-center">
          <p className="text-6xl m-2">Salaries</p>
        <ul className="flex flex-col justify-center items-center">{salaries.map((salary)=>{
                  return <SalaryCard salary={salary} key={salary._id}/> 
          })}</ul>
          <div>
